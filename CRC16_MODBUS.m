function [CRC16] = CRC16_MODBUS(puchMsg)
% function [output,CRC16] = CRC16_MODBUS(puchMsg) %原作者的output在运行output = [dec2hex(puchMsg);CRC16]时出现错误，串联的数组维度不一致，我想只输出crc16
...
%     output = [dec2hex(puchMsg);CRC16];%运行出现错误，串联的数组维度不一致，我想只输出crc16
end